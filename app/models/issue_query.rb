  ...
  def available_columns
    ...
    if User.current.allowed_to?(:view_time_entries, project, :global => true)
      ...
      @available_columns.insert index, QueryColumn.new(:total_spent_hours,
        :sortable => "COALESCE((SELECT SUM(hours) FROM #{TimeEntry.table_name} time_entry LEFT JOIN #{Issue.table_name} child_issue ON time_entry.issue_id = child_issue.id WHERE #{TimeEntry.table_name}.issue_id = #{Issue.table_name}.id OR child_issue.parent_id IS NOT NULL), 0)",        :default_order => 'desc',
